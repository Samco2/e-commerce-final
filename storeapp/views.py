...
def index(request):
    ...
    related =Product.objects.filter(top_deal=True)
    categories = Category.objects.all()
    products = Product.objects.all()
    context = {'top_deals': top_deal, 'categories':categories, 'products':products, 'flash_sales':flash_sales, 'related':related }
...
def detail(request, slug):
    ...
    try:
        ...
        counter = 1
        messages.info(request, f"{product.name} was added to cart")
    except:
        saveitem = None
    ...
    context = {'product': product, 'cart': cart, 'saveitem': saveitem,
               'counter': counter, 'recently_viewed_products':recently_viewed_products, 'similar_products': similar_products, 'categories':categories}
...
def cart(request):
    ...
    context = {'cart':cart, 'cartitems': cartitems, 'categories':categories}
...
def checkout(request):
    ...
    stripe.api_key = 'sk_test_51N4ayEGwmFNitjUhq7tt8Nu6RPfaHTawPQedKMbC3rpJBiVDIh1lqphmRUN1xs5z93wKiQ7RzfDWA5FNXg1KGUS800sgRV4K2Q'
...
def userdashboard(request):
    ...
    try:
        ...
        cus_address = None
    context = {'customer': customer, 'address':address, 'myaddress': cus_address, 'orders':orders}
    return render(request, 'storeapp/dashboard.html', context)
...
def confirmPayment(request):
    #paypal paid/returning amount
    data = json.loads(request.body)
    # Converting the amount to float
    total = float(data['total'])
    print(total)

    # Retrieving items total amount from backend data base
    cart = Cart.objects.get(session_id = request.session['nonuser'], completed=False)
    print(cart.cart_total)

    #Comparing paypal amount to data base total amount
    if total == cart.cart_total:
        # If equal, flag the order has completed order
        cart.completed = True
    else:
        #else, there is issue with the purchase
...
def search(request):
    ...
    context = {'products': products, 'search_query':search_query, 'categories': categories}
